...
bool FlightPathMovementGenerator::DoUpdate(Player* player, uint32 /*diff*/)
{
    ...
    if (pointId > i_currentNode)
    {
        ...
        do
        {
			if (i_currentNode >= i_path.size())
			{
				sLog->outPerformance("TAXI NODE WAS GREATER THAN PATH SIZE, GUID: %u, POINTID: %u, NODESIZE: %u, CURRENT: %u", player->GetGUIDLow(), pointId, i_path.size(), i_currentNode);
			...
			{
				sLog->outPerformance("Player on different map, curmap: %u, pointmap: %u, nodesize: %u, currentnode: %u", player->GetMapId(), i_path[i_currentNode]->mapid, i_path.size(), i_currentNode);
				player->CleanupAfterTaxiFlight();
				return false;
			}
